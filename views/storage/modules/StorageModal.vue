    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item label="名称" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input v-decorator="[ 'name', validatorRules.name]" placeholder="请输入名称" disabled></a-input>
        </a-form-item>
        <a-form-item label="库存" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input-number v-decorator="[ 'storage', validatorRules.storage]" placeholder="请输入库存" style="width: 100%" disabled/>
        </a-form-item>
  ...
  export default {
    ...
    data () {
      return {
        ...
        url: {
          edit: "/storageLog/storageLog/upReal",
        }
      }
    },
    created () {
      ...
      handleOk () {
        ...
        this.form.validateFields((err, values) => {
          if (!err) {
            ...
            var arr=[];
            arr.push(formData);
        getAction(this.url.edit,{items:encodeURI(JSON.stringify(arr))}).then((res)=>{
              if(res.success){
